...
export function VisualizationCanvas({
  ...
  return (
    <div className={`flex-1 flex flex-col min-w-0 ${isLight ? 'bg-[#f5f7ff]' : 'bg-[#050b14]'}`}>
      ...
      <div
        ref={containerRef}
        className={`flex-1 relative ${(dsType === 'hash_table' || dsType === 'trie' || dsType === 'segment_tree' || dsType === 'red_black_tree' || dsType === 'quick_sort') ? 'overflow-hidden' : 'overflow-hidden cursor-grab active:cursor-grabbing'}`}
      ...
      >
        ...
        {/* Content */}
        <div
          className={`${dsType === 'quick_sort' ? 'w-full h-full' : 'min-h-full'} flex items-center justify-center p-8`}
          style={{
      ...
      {/* ===== Bottom Toolbar ===== */}
      {/* Hide for hash_table, trie, segment_tree, red_black_tree, quick_sort - controls are in the visualization panel */}
